...
struct LiveChannelHistoryTests {
    ...
    @Test func `removing a channel drops it from the rail and the recall slot`() throws {
        ...
        try context.save()
        try LiveChannelHistory.record(media(forStreamId: 100, playlist: playlist, in: context), profileID: nil, defaults: defaults)
        try LiveChannelHistory.record(media(forStreamId: 102, playlist: playlist, in: context), profileID: nil, defaults: defaults)

        LiveChannelHistory.removeFromRecents(alpha, in: context, profileID: nil, defaults: defaults)
