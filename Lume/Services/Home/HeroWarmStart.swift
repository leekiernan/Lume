...
enum HeroWarmStartCache {
    /// Derived device-local cache metadata, not a user setting.
    ...
    /// the hero token itself: playlist changes, visibility/restriction changes,
    /// and an edited URL on the same custom section id.
    ...
    /// currently on screen. `catalogScope` covers the playlist, restrictions and
    /// custom source URL.
...
/// plumbing here means every section surface shares the same validation and
/// update behaviour.
